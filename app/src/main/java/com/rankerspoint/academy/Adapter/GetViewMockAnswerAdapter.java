...
public class GetViewMockAnswerAdapter extends RecyclerView.Adapter<GetViewMockAnswerAdapter.BannerImage> {
    List<GetViewMockAnswerModel> getPreEaxmModels;
    Context context;
    public static boolean LangFlag=true;
    List<GetViewMockAnswerModel> FinalList;
    String QuestionId="";
    String score1="";
    int UCts=0,CCts=0,WCts=0,score=0;
    String timeSet="",examid="",examName="";
    List<MyAnsResponse> MyAns=new ArrayList<>();
    public GetViewMockAnswerAdapter(Context context, List<GetViewMockAnswerModel> getPreEaxmModels){
    ...
    @NonNull
    @Override
    public GetViewMockAnswerAdapter.BannerImage onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.quiz_view_layout, parent, false);
        return new BannerImage(view);
    }

    @Override
    public void onBindViewHolder(@NonNull GetViewMockAnswerAdapter.BannerImage holder, int position) {
//        Log.e("tag", "onBindViewHolder: " + images.get(position));
        ...
        final GetViewMockAnswerModel getPreEaxmModel = FinalList.get(position);
    ...
    {
        FinalList.clear();
        GetViewMockAnswerModel FnModel=new GetViewMockAnswerModel();
            if (LangFlag) {
                for (GetViewMockAnswerModel model : getPreEaxmModels) {
                    FnModel=new GetViewMockAnswerModel();
