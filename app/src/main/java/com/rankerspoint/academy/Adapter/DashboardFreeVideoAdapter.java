...
public class DashboardFreeVideoAdapter extends RecyclerView.Adapter<DashboardFreeVideoAdapter.BannerImage> {
    List<DashBoardFreeVideoModel> getPreEaxmModels;
    Context context;

    public DashboardFreeVideoAdapter(Context context, List<DashBoardFreeVideoModel> getPreEaxmModels) {
    ...
    @NonNull
    @Override
    public DashboardFreeVideoAdapter.BannerImage onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.dashboard_video_layout_class_list, parent, false);
        return new BannerImage(view);
    }

    @Override
    public void onBindViewHolder(@NonNull DashboardFreeVideoAdapter.BannerImage holder, int position) {
//        Log.e("tag", "onBindViewHolder: " + images.get(position));
      ...
      //  picasso.load(images.get(position)).error(R.mipmap.ic_launcher).into(holder.img);
        ...
        final DashBoardFreeVideoModel getPreEaxmModel = getPreEaxmModels.get(position);
...
//        }
       ...
       Picasso.with(context).load(BaseUrl.BANN_IMG_URL + getPreEaxmModel.getPic()).into(holder.banner_image);
...
holder.card_liveVideo.setOnClickListener(new View.OnClickListener() {
    ...
    public void onClick(View view) {
        try {
            //  Log.d("linktype:--", freeVideoModel.getLinkType().toString());
            if (getPreEaxmModel.getType().trim().equals("Youtube")) {
                ...
                intent.putExtra("VIDEOURL", getPreEaxmModel.getVideoLink1().toString());
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
                context.startActivity(intent);
            } else if (getPreEaxmModel.getType().trim().equals("Normal")) {
                SharedPreferences sharedPref = context.getSharedPreferences("MyPref", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = sharedPref.edit();
                editor.putString("FeeStatusMentor", getPreEaxmModel.getFeeStatus());
                editor.apply();
                Intent intent = new Intent(context, EmbadedPlayer.class);
                intent.putExtra("TITLE", getPreEaxmModel.getName());
                intent.putExtra("CONTENTID",getPreEaxmModel.getDCRecordId());
                intent.putExtra("PIC",getPreEaxmModel.getPic());
                intent.putExtra("VIDEOURLNORMAL", getPreEaxmModel.getVideoLink1().toString());
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
                context.startActivity(intent);
            } else {
